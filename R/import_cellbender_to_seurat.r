...
read_cellbender_h5 <- function(filename) {
  ...
  tryCatch({
    ...
    ###
    # Extract metadata group if present
    if ("metadata" %in% names(h5_file)) {
      cat("Found metadata group\n")
      meta <- list()
      meta_group <- h5_file[["metadata"]]
      
      for (nm in names(meta_group)) {
        # Handle scalars, strings, or arrays safely
        meta[[nm]] <- tryCatch({
          meta_group[[nm]][]
        }, error = function(e) { NA })
      }
      
      qc_data$metadata <- meta
    }
  ###  
    h5_file$close_all()
...
import_cellbender_to_seurat <- function(cellbender_h5_path, 
  ...
  seurat_obj[["RAW"]] <- CreateAssay5Object(counts = raw_data_subset)
  ...
  seurat_obj@misc$cellbender_info <- list(
    cellbender_file = cellbender_h5_path,
    raw_file = raw_h5_path,
    import_date = Sys.time(),
    n_cells_raw = ncol(raw_data),
    n_cells_filtered = ncol(seurat_obj),
    n_genes_raw = nrow(raw_data),
    n_genes_filtered = nrow(seurat_obj)
  )
  
  ###
  # Add metadata from CellBender run if available
  if (!is.null(cb_qc$metadata)) {
    seurat_obj@misc$cellbender_info$metadata <- cb_qc$metadata
    cat("✓ Added metadata group to @misc$cellbender_info\n")
    cat("  Metadata fields:", paste(names(cb_qc$metadata), collapse = ", "), "\n")
  }
  ###
  
  # Add global ambient expression to existing cellbender_info if available
  if (!is.null(cb_qc$global_ambient_expression)) {
    features_cb <- rownames(cb_data)  # Get gene names from the matrix
    
    if (length(cb_qc$global_ambient_expression) == length(features_cb)) {
      # Create named vector with gene names
      ambient_named <- cb_qc$global_ambient_expression
      names(ambient_named) <- make.unique(features_cb)
      
      # Store in existing cellbender_info
      seurat_obj@misc$cellbender_info$ambient_expression <- ambient_named
      
      cat("✓ Added global ambient_expression to @misc$cellbender_info\n")
      cat("  Ambient expression vector length:", length(ambient_named), "\n")
    } else {
      cat("Warning: ambient_expression length doesn't match number of genes\n")
    }
  }
